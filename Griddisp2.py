...
class Griddisp2:
    ...
    def griddisp2(self,Prer,Pload,SoC,PES,start_diesel,Type,Disptch_mode,SoC_ref):  ## P_ES is power at POI
        ...
        Pnet=Prer+PES-Pload # The net power mismatch calculated as generation minus load       
        ## Load following & cycle charging
        if Type==1:  ## load following & cycle charging
            if Pnet>=0 and Disptch_mode==0:    # Generation more than load, wind is controllable, diesel is off, ESS dep#ends
                    ...
                    if SoC>=self.SoC_max:     # SoC is larger or equal to maximum SoC
                        ...
                        self.Pessref=0      # Power refernece of ESS is 0
                        ...
                        if start_diesel>=1  and  start_diesel<=(self.Tdiesel-1): # The number of intervals of diesel ON is between 1 and self.Tdiesel-1
                            self.Pdsref=self.Pds_min*1   # Power reference of diesel is at mimimum
                            if (Pnet+self.Pdsref)<self.Pch_max:  # Net power mismatch plus diesel power is less than maximum charge power of ESS
                                self.Pessref=Pnet+self.Pds_min # ESS will be charged at Pnet+self.Pds_min
                                ...
                                if self.Start_ds>=self.Tdiesel:  # if start_diesel larger than the minimum required time
                                    self.Start_ds=0       # Diesel turned OFF
                                #end
                            else:                      # (Pnet+self.Pdsref)>=self.Pch_max
                                self.Pessref=self.Pch_max      # ESS been charged at the maximum charge power
                            ...
                            if Pnet<self.Pch_max:           # Net power smaller than maximum charge power of ESS
                                self.Pessref=Pnet         # Charge ESS with net power
                                self.Prerref=0
                                self.Pdsref=0
                                self.Pldref=0
                            else:                      # Net power larger than maximum charge power of ESS
                                self.Pessref=self.Pch_max      # Charge ESS with net power
                        ...
                        #end
                    #end            
            elif SoC>=self.SoC_min  and  Disptch_mode==0:       # Pnet<0 (load larger than generation), SoC>=self.SoC_min and self.disp_mode is 0
                     self.disp_mode=0                     # self.disp_mode kept at 0
                     if  start_diesel>=1  and  start_diesel<=(self.Tdiesel-1): # The number of intervals of diesel ON is between 1 and self.Tdiesel-1
                           Pnet=Prer+self.Pds_min+PES-Pload  # self.Pdsref=self.Pds_min, readjust Pnet to include self.Pds_min
                           if -Pnet<-self.Pdis_max:         # Pnet is smaller than largest discharge power
                                self.Pessref=Pnet         # ESS discharged power at Pnet
                                ...
                                self.Pessref=self.Pdis_max       # ESS discharge power at its maximum
                            ...
                            self.Start_ds=0                  # Diesel OFF
                            if -Pnet<-self.Pdis_max:           # Pnet is smaller than largest discharge power
                                self.Pessref=Pnet            # ESS Power reference is net power
                                self.Pldref=0
                                self.Pdsref=0
                                self.Prerref=0
                            else:                        # Pnet is larger than the largest discharge power
                                self.Pessref=self.Pdis_max       # ESS will discharge power at its maximum
                                self.Pldref=-Pnet+self.Pdis_max  # Load shed to maintain power balance
                                self.Pdsref=0
                                self.Prerref=0
                            #end
                     ...
                     if -Pnet<self.Pds_min:                 # Pnet is smaller than self.Pds_min
                            if  start_diesel>=1  and  start_diesel<=(self.Tdiesel-1): # The number of intervals of diesel ON is between 1 and self.Tdiesel-1
                                ...
                                self.Start_ds=start_diesel+1
                                self.Pldref=0             # Load shedding is 0
                                self.Pessref=self.Pds_min+Pnet # ESS to maintain power balance
                                ...
                                self.Prerref=0
                            #end
                     else:  # Pnet is larger than self.Pds_min, self.Pdsref=self.Pds_max                 
                            if (-Pnet-self.Pds_max)<=0:     # self.Pds_min<-Pnet<self.Pds_max
                                self.Pdsref=self.Pds_max       # Diesel at maximum to charge BESS back
                                self.Pessref=self.Pds_max+Pnet # ESS to maintian power balance
                                self.Start_ds=start_diesel+1
                                self.Prerref=0             # Wind curtailment is 0
                                self.Pldref=0             # Load shedding is 0
                            else:                      # -Pnet>self.Pds_max
                                self.Pessref=0            # ESS power reference is 0
                                self.Prerref=0             # Wind curtail is 0
            ...
            if ((self.SoCref1<SoC and SoC<=SoC_ref) and self.disp_mode!=1) or Disptch_mode==2:  ##should disable or in type D, enable in type E
                ...
                Pnet = -Prer-PES+Pload             # Pnet recalcuated as load minus genertion
                self.Start_ds=start_diesel+1                 # Diesel is ON, self.Start_ds increased by one
                if (Pnet-self.Pds_max)>=0:                 # Net power larger than maximum diesel power
                    if (Pnet-self.Pds_max)<-self.Pdis_max:      # (Net power with maximum diesel power) smaller than largest discharge power
                        ...
                        self.Pldref=0
                        self.Prerref=0
                        self.Pessref=-Pnet+self.Pds_max       # ESS discharge to maintain power
                        ...
                        self.Prerref=0
                        self.Pessref=self.Pdis_max            # ESS discharges maximum power
                    #end
                    ...
                    self.Pldref=0
                    if (self.Pdsref-Pnet)<self.Pch_max:         # Remaining power to be balanced smaller than maximum charging power
                        self.Prerref=0                    # wind power curtail is 0
                        self.Pessref=self.Pdsref-Pnet         # ESS used to maintain power balance
                        if self.Pessref>=0:                # ESS is to charge
                            self.Pldref=0
                        else:                         # ESS is to discharge
                            self.Pldref=-self.Pessref         # Lod shed instead of ESS discharging 
                            self.Pessref=0               # ESS power set at 0            
                        #end
                    else:                             # Remaining power larger than maximum charging power
                        self.Pessref=self.Pch_max             # ESS charged at maximum charging power
                        self.Prerref=-Pnet+self.Pdsref-self.Pch_max # Wind curtailed to maintain power balance
                    #end
                #end
            else:                         ## Not in power-smoothing: load following or cycle charging
                Pnet=Prer+PES-Pload                # Pnet calculated as generation minus load
            if Pnet>=0 and Disptch_mode==0:                 # Load larger than generation and self.disp_mode=0
                    self.disp_mode=0                     # self.disp_mode kept at 0
                    if SoC>=self.SoC_max:     # # SoC is larger or equal to maximum SoC
                        self.Prerref=Pnet                 # Wind curtail power is the net power 
                        self.Pessref=0                   # Power refernece of ESS is 0
                        ...
                        if start_diesel>=1  and  start_diesel<=(self.Tdiesel-1): # The number of intervals of diesel ON is between 1 and self.Tdiesel-1
                            ...
                            if (Pnet+self.Pdsref)<self.Pch_max: # Net power mismatch plus diesel power is less than maximum charge power of ESS
                                ...
                                self.Pldref=0            # Load power shedding is 0
                                self.Pdsref=self.Pds_min
                                self.Start_ds=start_diesel+1 # The number of intevals that Diesel ON increased by one
                            else:                     # (Pnet+self.Pdsref)>=self.Pch_max
                                self.Pessref=self.Pch_max     # ESS been charged at the maximum charge power
                                self.Prerref=Pnet+self.Pdsref-self.Pch_max # Wind power curtailed to maintain power balance
                                self.Pldref=0            # Load shed is zero
            ...
            if Pnet>=0:    # Load more than generation, wind is controllable, diesel is off, ESS dep#ends
                    if SoC>=self.SoC_max:    # SoC is larger or equal to maximum SoC
                        self.Prerref=Pnet    # Wind curtail power is the net power
                        self.Pessref=0      # Power refernece of ESS is 0
                        self.Pdsref=0       # Diesel is OFF
                     ...
                     if -Pnet<self.Pds_min:  # Pnet is smaller than self.Pds_min
                            if  start_diesel>=1  and  start_diesel<=(self.Tdiesel-1):
                                ...
                                self.Start_ds=0     # Keep diesel OFF
                                self.Pdsref=0       # Diesel output 0
                                self.Pldref=-Pnet   # Load shed to maintain power balance
                                self.Pessref=0      # ESS power is zero
                                self.Prerref=0       # Wind curtail is zero
                            #end
                     else:                   
                            if (-Pnet-self.Pds_max)<=0:  # self.Pds_min<-Pnet<self.Pds_max
                                self.Pessref=0         # ESS power reference at 0
                                self.Pdsref=-Pnet      # Diesel used to balance net power
                                self.Start_ds=start_diesel+1 # Number of interval diesel ON increased by one
                                self.Prerref=0
                                self.Pldref=0
            ...
            ## Power smoothing 
                ...
                Pnet = -Prer-PES+Pload  # Net power is calculated as load minus generation
                self.Start_ds=start_diesel+1      # Diesel is ON all the time
                if (Pnet-self.Pds_max)>=0:     # Net power larger than maximum diesel power
                    if (Pnet-self.Pds_max)<-self.Pdis_max:      # (Net power with maximum diesel power) smaller than largest discharge power
...
#                         self.Pldref=0
                        self.Prerref=0
#                         self.Pessref=-Pnet+self.Pds_max # ESS to maintain discharge
                    ...
                    self.Pdsref=self.Pds_min   # Diesel output minmum diesel power
                    if self.Start_ds>=self.Tdiesel: # Number of diesel ON intervals larger than self.Tdiesel
                        self.Start_ds=0   # Diesel ON intervals set to zero
                        self.Pdsref=0     # Diesel turned OFF             
                    #end
                    self.Pldref=0         # Load shedding is 0
                    if (self.Pdsref-Pnet)<self.Pch_max: # Remaining power to be balanced smaller than maximum charging power
                        self.Prerref=0            # wind power curtail is 0
                        ...
                        self.Prerref=-Pnet+self.Pdsref-self.Pch_max # Wind curtailment 
                        self.Pessref=self.Pch_max   # ESS charged at maximum charging power
                    #end
                #end
        #end
